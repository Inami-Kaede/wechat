package com.hayate.algorithm.graph;

import java.util.Arrays;
import java.util.LinkedList;

public class Graph2 {

	private int vertexSize; //顶点数量
	private int[] vertexs; //顶点数组
	private int[][] matrix; //矩阵
	private static final int MAX_WEIGHT = 1000; //最大权值
	
	private boolean[] isVisited;
	
	public Graph2(int vertexSize) {
		super();
		this.vertexSize = vertexSize;
		matrix = new int [vertexSize][vertexSize];
		vertexs = new int [vertexSize];
		for(int i = 0; i < vertexSize; i++){
			vertexs[i] = i;
		}
		isVisited = new boolean[vertexSize];
	}
	
	public int[] getVertexs() {
		return vertexs;
	}
	
	public void setVertexs(int[] vertexs) {
		this.vertexs = vertexs;
	}
		
	/**
	 * 获取出度
	 * @param index
	 * @return
	 */
	public int getOutDegree(int index){
		int degree = 0;
		for(int j = 0; j < matrix[index].length; j ++){
			int weight = matrix[index][j];
			if(weight != 0 && weight != MAX_WEIGHT){
				degree++;
			}
		}
		return degree;
	}
	
	/**
	 * 获得权值
	 * @param v1
	 * @param v2
	 * @return
	 */
	public int getWeitht(int v1, int v2){
		int weight = matrix[v1][v2];
		return weight == 0 ? 0 : (weight == MAX_WEIGHT ? -1 : weight);
	}
//////////////////////////////////////////////////////////////////////////////////////	

	/**
	 * 获取某个顶点的第一个邻接点
	 * @param vertexNo
	 * @return
	 */
	public int getFirstNeighbor(int vertexNo){
		for(int j = 0; j < vertexSize ; j++){
			if(matrix[vertexNo][j] > 0 && matrix[vertexNo][j] < MAX_WEIGHT){
				return j;
			}
		}
		return -1;
	}
	
	/**
	 * 根据前一个邻接点的下标来取得下一个邻接点
	 * @param vertexNo 表示要找的顶点
	 * @param preCrossVertexNo 表示该顶点相对于哪个邻接点去获取下一个邻接点
	 * @return
	 */
	public int getNextNeighbor(int vertexNo, int preCrossVertexNo){
		for(int j = preCrossVertexNo + 1; j< vertexSize; j++){
			if(matrix[vertexNo][j] > 0 && matrix[vertexNo][j] < MAX_WEIGHT){
				return j;
			}
		}
		return -1;
	}
	
	/**
	 * 图的深度优先遍历算法
	 * @param vertexNo
	 */
	public void depthFirstSearch(int vertexNo){
		isVisited[vertexNo] = true;
		int crossTargetNo = getFirstNeighbor(vertexNo);
		while(crossTargetNo != -1){
			if(!isVisited[crossTargetNo]){
				//需要遍历该顶点
				System.out.println("访问到了："+crossTargetNo+"顶点");
				depthFirstSearch(crossTargetNo);
			}
			crossTargetNo = getNextNeighbor(vertexNo, crossTargetNo);
		}
	}
	
	/**
	 * 对外公开的深度优先遍历（主要针对有向图）
	 */
	public void depthFirstSearch(){
		isVisited = new boolean[vertexSize];
		for(int i = 0; i < vertexSize; i ++){
			if(!isVisited[i]){
				System.out.println("访问到了："+i+"顶点");
				depthFirstSearch(i);
			}
		}
		isVisited = new boolean[vertexSize];
	}
	
	public void broadFirstSearch(){
		isVisited = new boolean[vertexSize];
		for(int i = 0; i < vertexSize; i ++){
			if(!isVisited[i]){
				broadFirstSearch(i);
			}
		}
		
	}

	/**
	 * 广度优先遍历
	 * @param vertexNo
	 */
	private void broadFirstSearch(int vertexNo) {
		int temp,crossTargetNo;
		LinkedList<Integer> queue = new LinkedList<Integer>();
		System.out.println("访问到了："+vertexNo+"顶点");
		isVisited[vertexNo] = true;
		queue.add(vertexNo);
		while(!queue.isEmpty()){
			temp = queue.removeFirst().intValue();
			crossTargetNo = getFirstNeighbor(temp);
			while(crossTargetNo != -1){
				if(!isVisited[crossTargetNo]){
					System.out.println("访问到了："+crossTargetNo+"顶点");
					isVisited[crossTargetNo] = true;
					queue.add(crossTargetNo);
				}
				crossTargetNo = getNextNeighbor(temp, crossTargetNo);
			}
		}
	}
	
	/**
	 * 普里姆算法
	 */
	public void prim(){
		int [] lowcost = new int[vertexSize];//最小代价顶点权值的数组,为0表示已经获取最小权值
		int [] adjvex = new int[vertexSize];//放顶点权值
		int min,minId,sum = 0;
		//初始化第一列
		for(int i = 1;i<vertexSize;i++){
			lowcost[i] = matrix[0][i];
		}
		for(int i = 1;i<vertexSize;i++){
			min = MAX_WEIGHT;
			minId = 0;
			for(int j = 1;j<vertexSize;j++){
				if(lowcost[j]<min&&lowcost[j]>0){
					min = lowcost[j];
					minId = j;
				}
			}
			System.out.println("顶点："+adjvex[minId]+"权值："+min);
			sum+=min;
			lowcost[minId] = 0;
			for(int j = 1;j<vertexSize;j++){
				if(lowcost[j]!=0&&matrix[minId][j]<lowcost[j]){
					lowcost[j] = matrix[minId][j];
					adjvex[j] = minId;
				}
			}
		}
		System.out.println("最小生成树权值和:"+sum);
	}
	
	public void myPrim(){
		int sum = 0;
		int [] temp = new int[vertexSize];
		int [] index = new int[vertexSize]; //记录连接(到)顺序0,1,5,8,2,6,7,4,3,
		int [] total = new int[vertexSize]; //记录对应连接的权值0,10,11,12,8,16,19,7,16,
		for(int i=1;i<vertexSize;i++){
			temp[i] = matrix[0][i];
		}
		int minId = 0;
		for(int j=1;j<vertexSize;j++){
			int min = MAX_WEIGHT;
			for(int i=1;i<vertexSize;i++){
				if(0<temp[i]&&temp[i]<min){
					min = temp[i];
					minId = i;
				}
			}
			System.out.println("当前最小值："+min);
			sum += min;
			temp[minId] = 0;
			
			index[j] = minId;
			total[j] = min;
			//合并操作
			for(int i=1;i<vertexSize;i++){
				if(temp[i]!=0&&matrix[minId][i]<temp[i]){
					temp[i] = matrix[minId][i];
				}
			}
		}
		System.out.println("最小生成树权值和："+sum);
		Arrays.stream(index).forEach(i->System.out.print(i+","));
		System.out.println();
		Arrays.stream(total).forEach(i->System.out.print(i+","));
	}
	
	public static void main(String[] args) {
		
		Graph2 graph = new Graph2(9);
		int [] a1 = {0,10,MAX_WEIGHT,MAX_WEIGHT,MAX_WEIGHT,11,MAX_WEIGHT,MAX_WEIGHT,MAX_WEIGHT};
		int [] a2 = {10,0,18,MAX_WEIGHT,MAX_WEIGHT,MAX_WEIGHT,16,MAX_WEIGHT,12};
		int [] a3 = {MAX_WEIGHT,MAX_WEIGHT,0,22,MAX_WEIGHT,MAX_WEIGHT,MAX_WEIGHT,MAX_WEIGHT,8};
		int [] a4 = {MAX_WEIGHT,MAX_WEIGHT,22,0,20,MAX_WEIGHT,MAX_WEIGHT,16,21};
		int [] a5 = {MAX_WEIGHT,MAX_WEIGHT,MAX_WEIGHT,20,0,26,MAX_WEIGHT,7,MAX_WEIGHT};
		int [] a6 = {11,MAX_WEIGHT,MAX_WEIGHT,MAX_WEIGHT,26,0,17,MAX_WEIGHT,MAX_WEIGHT};
		int [] a7 = {MAX_WEIGHT,16,MAX_WEIGHT,MAX_WEIGHT,MAX_WEIGHT,17,0,19,MAX_WEIGHT};
		int [] a8 = {MAX_WEIGHT,MAX_WEIGHT,MAX_WEIGHT,16,7,MAX_WEIGHT,19,0,MAX_WEIGHT};
		int [] a9 = {MAX_WEIGHT,12,8,21,MAX_WEIGHT,MAX_WEIGHT,MAX_WEIGHT,MAX_WEIGHT,0};
		graph.matrix[0] = a1;
		graph.matrix[1] = a2;
		graph.matrix[2] = a3;
		graph.matrix[3] = a4;
		graph.matrix[4] = a5;
		graph.matrix[5] = a6;
		graph.matrix[6] = a7;
		graph.matrix[7] = a8;
		graph.matrix[8] = a9;
		
		//graph.depthFirstSearch();
//		graph.broadFirstSearch();
//		graph.prim();
		graph.myPrim();
		
	}
}
