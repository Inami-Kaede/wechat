...
public class GraphKruskal {
	...
	public int mySearch(int [] temp, int value){
		while(temp[value] > 0){
			value = temp[value];
		}
		return value;
	...
	public void myKruskal(){
		int[] temp = new int[edgeSize];
		int begin,end,sum = 0;
		for(int i = 0; i < edgeSize; i++){
			begin = mySearch(temp, edges[i].begin);
			end = mySearch(temp, edges[i].end);
			if(begin != end){
				temp[begin] = end;
				System.out.println("从 "+edges[i].begin+" 连接到 "+edges[i].end+" 权值为 "+edges[i].weight);
				sum += edges[i].weight;
			}
		}
		System.out.println("最小生成路径为："+sum);
	}
